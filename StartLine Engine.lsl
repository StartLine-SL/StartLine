...
handle_collision(integer num, integer diff, integer now) {
    ...
    string strId  =llGetSubString(name, idStrIdx+1, idStrIdx+6);  //boat id 6 characters
    ...
    if(!fwdCrossing(vel)){   //cross in the opposite direction
        if(lineMode==1){  //match mode
            if(diff<0){   //if pre start
                diff=llAbs(diff);
                sw=0;
                if(diff>time3) llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg2",ownerDName+"  ID"+strId+" has crossed early!");
                else if(diff<=time1) llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg2",ownerDName+"  ID"+strId+" has entered the pre-start zone too late you are DSQ");
                else{
                    if(sailor1!=(string)ownerKey+strId && sailor2!=(string)ownerKey+strId && (sailor1=="" || sailor2=="")){
                        if(diff>time2){ 
                            llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg2",ownerDName+"  ID"+strId+" has entered the pre-start zone");
                            sw=1;
                        }else if(diff>time1){ 
                            llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg2",ownerDName+"  ID"+strId+" has entered the pre-start zone out of time");
                            sw=1;
                        }             
                        if(sw==1){
                            if(sailor1=="") sailor1=(string)ownerKey+strId;
                            else if(sailor2=="") sailor2=(string)ownerKey+strId;
                        }
                    }
                }
                return;
            ...
            llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg",ownerDName+"  ID"+strId+" wrong way!");
            llMessageLinked(LINK_ALL_OTHERS,6000,"chanmsg","badir|"+(string)ownerKey+"|"+strId+"|"+(string)ways);
            return;
        }
    }
    if(lineMode!=3 && diff<0){ //diff is not valid for multistart  chronometer<0
        if(lineMode==2 || lineMode==7){  //against time or trial
            integer listIdx=llListFindList(ownerList,[(string)ownerKey+strId]);  //look in the list
            if(listIdx>=0){   //find it 
                if(llList2Integer(timeList,listIdx)>0) return;  //if the time is negative, cross back one lap and continue
            }else return;  //can't find it  cross out of time
        }else{ //over early
            llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg",ownerDName+"  ID"+strId+" is over early!  Go around and recross!");
            llMessageLinked(LINK_ALL_OTHERS,6000,"chanmsg","early|"+(string)ownerKey+"|"+strId+"|"+(string)diff+"|"+(string)now);        
            return;
        }
    }
    ...
    string s=ownerDName+","+ownerName+","+(string)ownerKey+","+(string)diff+","+(string)now+","+strType+","+strId;
...
handle_crossing(string list_argument) {
    ...
    string boatType=llList2String(parse,5);
    string raceID=llList2String(parse,6);
    ...
    if(listIdx==-1){ //not in list... will add to list if not over early
        ...
        if(lineMode==3){
            ...
            if(raceTimeRel<0){
                llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg",ownerDName+"  ID"+raceID+" is over early!  Go around and recross!");
                llMessageLinked(LINK_ALL_OTHERS,6000,"chanmsg","early|"+(string)ownerKey+"|"+raceID+"|"+(string)raceTimeRel+"|"+(string)raceTimeAbs);        
                return;
            }                
        } else if(lineMode==6) {
            if(nettype!="S") return;
        ...
        if(llGetFreeMemory()>2000){
            ...
            if(isCallsValue(1,1,0)) llMessageLinked(LINK_THIS,-22011,(string)raceTimeRel+","+raceID,ownerKey);    //call  
            ...
            llMessageLinked(LINK_ALL_OTHERS,6000,"chanmsg","avsta|"+(string)ownerKey+"|"+raceID+"|"+(string)raceTimeRel);
        }else{
            llMessageLinked(LINK_ALL_OTHERS,6011,"sendmsg2","");
        }
        return;
    } 
        ...
        if (llFabs(llList2Float(timeList,listIdx)-raceTimeRel)>minTime) { //is time dif > min lap time?
            ...
            if (lapNum<=raceNumLaps) {
                ...
                if (lapNum==raceNumLaps) {
                    ...
                    llMessageLinked(LINK_ALL_OTHERS,6000,"chanmsg","avend|"+(string)ownerKey+"|"+raceID+"|"+snumorder+"|"+(string)lapTime+"|"+(string)raceTimeRel);        
                } else {
                    ...
                    llMessageLinked(LINK_ALL_OTHERS,6000,"chanmsg","avlap|"+(string)ownerKey+"|"+raceID+"|"+(string)lapNum+"|"+(string)lapTime+"|"+(string)raceTimeRel);   
                }
                if(msg!=""){
                    llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg","/me : "+msg);        
                }
            } else {
    ...
    if(lineMode==2 || lineMode==7) { //in the list  against time and trail mode
        ...
        if(lapTime<0){ // If time is negative, it is the start time of the lap that is now ending
        ...
        } else if(lineMode==2) {  //against time mode start a new lap
            if(llList2Integer(lapList,listIdx)<9){
                ...
                if(isCallsValue(1,1,1)) llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg",ownerDName+"  ID"+raceID+" starts new lap");
            }else{ 
    ...
    if(lineMode==3){ //in the list, so they must have started already  Multistart
        dbgSay("MultiStart Found racer "+ownerName+" in list. Checking for lap time.");
        ...
        if (llFabs(llList2Float(timeList,listIdx)-raceTimeRel)>minTime) { //is time dif > min lap time?
            ...
            if (lapNum<=raceNumLaps) {
                ...
                if (lapNum==raceNumLaps) {
                    ...
                    if(isCallsValue(1,2,0)) llMessageLinked(LINK_THIS,-22012,(string)lapNum+","+(string)lapTime+","+(string)raceTimeRel+","+raceID,ownerKey);    //call 
                    if(isCallsValue(1,2,1)) msg=ownerDName+"  ID"+raceID+": "+(string)lapNum+" lap(s) completed at time "+sec2hhmmss(lapTime)+" -- Laptime is "+sec2hhmmss(raceTimeRel);
                    llMessageLinked(LINK_ALL_OTHERS,6000,"chanmsg","avlap|"+(string)ownerKey+"|"+raceID+"|"+(string)lapNum+"|"+(string)lapTime+"|"+(string)raceTimeRel);   
                }
                if(msg!=""){
                    llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg2",msg);        
    ...
    if(lineMode==4) { //in the list  race test
        ...
        if(lapNum==0){ //finish lap
            ...
            if(isCallsValue(1,1,1)) llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg",ownerDName+"  ID"+raceID+" starts at time "+sec2hhmmss(raceTimeRel)); 
        }
    }
}
...
addRacer(string ownername, key owner, integer startTimeRel, integer startTimeAbs, string boatType, string raceID) {
    ...
    lapList+=[0];
    if(lineMode==2 || lineMode==7){  //against time
        ...
        if(lineMode==6){
            if(nNetSend<0) llMessageLinked(LINK_ALL_OTHERS,6000,"netsend2",(string)owner+raceID+"#"+(string)startTimeRel+"#"+ownername+" "+raceID);
        }
    }
    ownerList+=[(string)owner+raceID];
    nameList+=ownername+" "+raceID;
    llMessageLinked(LINK_ALL_OTHERS, 6901, ownername+" "+raceID, "");
}
...
{
    if (swSend==0) llMessageLinked(LINK_ALL_OTHERS,6000,"sendmsg","/me :"+results);   
...
default 
{
    ...
    link_message(integer sender_num, integer num, string str, key id)
    {
        ...
        if (str=="results") {
            ...
            if(lineMode==1) llMessageLinked(LINK_ALL_OTHERS,6801,llList2CSV(lapList),llList2CSV(timeList));  //1-match
            else if(lineMode==2) llMessageLinked(LINK_ALL_OTHERS,6802,"","");   //against
            else if(lineMode==3){    //multistart
            ...
            }else if(lineMode==4) llMessageLinked(LINK_ALL_OTHERS,6804,llList2CSV(lapList),llList2CSV(timeList));   //race training
            else if(lineMode==5) llMessageLinked(LINK_ALL_OTHERS,6805,"",llList2CSV(timeList));  //start training
            else if(lineMode==6) llMessageLinked(LINK_ALL_OTHERS,6806,llList2CSV(lapList),llList2CSV(timeList));  //6-network
            else if(lineMode==7) llMessageLinked(LINK_ALL_OTHERS,6807,"","");  //Trial
            else llMessageLinked(LINK_ALL_OTHERS,6800,(string)raceNumLaps+llList2CSV(lapList),llList2CSV(timeList));  //0-race
            return;
        }
        if (lineMode==6){
            if (str=="netsend"){  //from startline script
                integer nn=llGetListLength(ownerList);
                if(nn<=netIndex || nNetSend==-1){ 
                    nNetSend=-1;
            ...
            } else if (str=="netrdata"){
                list l=llParseString2List((string)id,["#"],[""]);
                integer vt=llGetListLength(ownerList);
                ownerList+=llCSV2List(llList2String(l,0));
                nameList+=llCSV2List(llList2String(l,2));
