...
DisplayMenu(string ps1, string ps3)   //main menu  page one
{
    ...
    if (llList2String(ls3,3)=="0") {   //if swRun 
    ...
    } else if(llList2String(ls1,7)=="1" && llList2Key(ls3,1)!=NULL_KEY && llList2Key(ls3,1)!=llList2Key(ls3,0) && isowner==0) {   //unlock
    ...
    } else if (llList2String(ls1,7)=="2" && isowner==0){   //lock and is not owner
        menu=["Close Menu"];
        ...
        else if(llList2String(ls1,2)=="2" || llList2String(ls1,2)=="7") s1="Race Time...";   //mode against time  and trial
        ...
        //menu=["Mode","Help","❯❯❯❯",s1,s2,"Load Default","Results","Num.Laps","Direction","Close Menu",s0,"Finish/Restart"];
...
string viewSettings(string ps1, integer pisowner)
{
    ...
    if(llList2String(ls1,2)=="2") s+="\nAgainst Time mode Race Time: "+sec2hms((integer)llList2String(ls1,10)); 
...
//ps1=0-raceNumLaps,1-ways,2-lineMode,3-set_time,4-visible,5-minTime,6-setLockTime,7-lockType,8-lockName,9-locktime,10-raceTime,11-startNum,
//        12-matchOpen,matchClose,matchBad,15-nameMode,16-nettype,17-netname,18-netconnect,19-netlinkRegion,20-setLockTimeAdmin
//ps2=0-type,1-pId,2-lockID,3-plugname,4-startRez,5-lineLen;
SubMenu(string ps1, string ps2)
{
    ...
    if(type=="opt"){  //main menu pag two
        ...
        if(llListFindList(owners,[(key)llList2String(ls2,1)])>=0){    
            menu=["Hard Lock","Line Length","Load Texture","Save Opt.","Reset","Unlock.","Up."];
            if (model==1){ 
        ...
        menu=["90 sec.","150 sec.","210 sec.","0 sec.","30 sec.","60 sec.","Up."];
        ...
        if(llList2String(ls1,2)=="7" && isowner){ 
            menu=["Others...","  ","  ","60 min","90 min","120 min","Up.","30 min","45 min"];
        ...
        menu=["Against Time","Start Training","Network","Match Race","Race Training","Multi Start","Up","Race","Trial"];
        ...
        string vs1="Waiting";
        if (swWaiting) vs1="Stop Waiting";
        //menu=["Net Link...","Get LinkCode", "Put LinkCode", "Start Line", "Finish Line", "  ", "Up", vs1, "Connect"];
        menu=["Net Link...","Get LinkCode", "Put LinkCode", "Up", vs1, "Connect"];
        ...
        s+="StartLine Network Links menu";
        list menu=listLinkN+"Up";
    ...
    listen(integer pchannel, string name, key id, string cmd) 
    {
        if (pchannel!=dialogChannel){ 
            if (pchannel==textboxChannel) {
                ...
                if (lineMode==6) {
                    cmd=llGetSubString(cmd,0,7)+"-"+llGetSubString(cmd,8,11)+"-"+llGetSubString(cmd,12,15)+"-"+llGetSubString(cmd,16,19)+"-"+llGetSubString(cmd,20,31);
                    if ((key)cmd) {
                        fOption(1941,"",(key)cmd);
                        llMessageLinked(LINK_THIS, 7100, "netcodeok", id);
                    } else {
                        llMessageLinked(LINK_THIS, 7100, "netcodebad", id);
                    }
                }else if (lineMode==7) {
        ...
        if(lockType>0){
            if (lockType==1 && cmd=="Unlock") {
                fOption(1910,"unlocksoft",id);
                return;
            }
            if (cmd=="Unlock.") {
                if (idxowner>=0){
                    fOption(1910,"unlockhard",id);
                }
                return;
            }
            if (lockID!=id) {
                fOption(1910,"unlockbad",id);
                return;
            }
        }
        ...
        if (cmd=="CountDown...") { fIsRun(0, "time", id); return; }
        ...
        if (cmd=="Results" || cmd=="Results...")  { 
            if(lineMode==3) fSubMenu("results",id);
            else fOption(1912,"",id);
            return;
        }        
        if (cmd=="Start")  {
            if(model==1 && startRez==0) llMessageLinked(LINK_THIS, 7101, "", id);    //First you have to press the 'Build Line' option     
            else fIsRun(1913, "", id);
            return;
        }            
        if (cmd == "Finish/Restart") { fOption(1914,"",id); return; }
        if (cmd == "Reset") { fOption(1915,"",id); return; }
        if(cmd=="Save Opt.") { fOption(1916,"",id); return; } 
        if (cmd == "Help.") { 
        ...
        if (cmd=="-" || cmd=="--" || cmd=="---" || cmd=="+" || cmd=="++" || cmd=="+++") {           //1923 free**
            llMessageLinked(LINK_THIS, 3000, "adjlength", cmd);   //for model 0 and model 1                
            fSubMenu("adjustline",id);
            return;
        }
        if (cmd == "Race Time...") { fIsRun(0, "racetime", id); return; }                                                                //1924 free** 
        if (cmd == "Load Texture") { llMessageLinked(LINK_THIS, 3000, cmd, id); return; }   
        if (cmd == "Race") { fMode("0", cmd, id); return; }
        if (cmd == "Match Race") { fMode("1", cmd, id); return; }
        if (cmd == "Against Time") { fMode("2", cmd, id); return; }
        ...
        if (llGetSubString(cmd,-2,-1) == "m.") {            //line length
            llMessageLinked(LINK_THIS, 3000+(integer)llGetSubString(cmd,0,2), "setlength", id);   
            fSubMenu("linelength",id);
            return; 
        }     

        if(lineMode==1){   //against time
        ...
        if(lineMode==2){   //against time
            if (llGetSubString(cmd,-8,-1)==" minutes") { fOption(1933,llGetSubString(cmd,0,-8),id); return; } //race time    against time 
        }
        if(lineMode==3){  //multistart
            ...
            if (llGetSubString(cmd,0,5)=="Start-") { llMessageLinked(LINK_ALL_OTHERS,5000,"startchron",llGetSubString(cmd,6,6)); }
        ...
        if(lineMode==6){   //network
            ...
            if (cmd == "Net Link...") { fSubMenu("netlink",id); return; }
            if (cmd == "Connect") { fOption(1940,"",id); return; }     //1939 free**
            if (cmd == "Get LinkCode") { llMessageLinked(LINK_ALL_OTHERS,6000,"getcode",id); return; }
            if (cmd == "Put LinkCode") {
                llListenRemove(textboxHandle);
                textboxHandle = llListen(textboxChannel, "", "", "");
                llTextBox(id ,"Enter the code of the line to link:" , textboxChannel);
                if(menutimeout>0) llSetTimerEvent(menutimeout);
                return;
            }
            if (cmd == "Waiting") { fOption(1942,"",id); return; }
            if (cmd == "Stop Waiting") { fOption(1943,"",id); return; }
            if (nListLinkN>0){
                if(llGetSubString(cmd,0,2)=="NL-"){
                    integer nlink=llListFindList(listLinkN,[cmd]);
                    if(nlink>=0) {
                        fOption(1944,cmd,(key)llList2String(listLinkK,nlink));
                        llMessageLinked(LINK_THIS, 7100, "netlink", (string)id+llGetSubString(cmd,3,-1));
                    }
                    return;
                }
            }
        ...
        if(lineMode==7){   //trail mode
            ...
            if (cmd == "Others...") {
                llListenRemove(textboxHandle);
                textboxHandle = llListen(textboxChannel, "", "", "");
                llTextBox(id ,"Enter time in minutes (15min - 1800min):" , textboxChannel);
                if(menutimeout>0) llSetTimerEvent(menutimeout);
                return;
            }
    ...
    link_message(integer sender_num, integer num, string str, key data) 
    {
        if (num>=0 && num<1000) {   //link messages for all scripts
            ...
            if (str=="setopt") {
                ...
                model=(integer)llList2String(l,4);
        ...
        if (str=="netlinkn") {  
            listLinkN=llCSV2List(data);   //net link name list
            nListLinkN=llGetListLength(listLinkN);
            return;
        }
        if (str=="netlinkk") {  
            listLinkK=llCSV2List(data);   //net link keys list
            return;
        }
    }
